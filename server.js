...
const allowedOrigins = [
  ...
  "https://whether.oscarmcglone.com",
];
...
app.post("/vote", async (req, res) => {
    ...
    try {
        ...
        const range = `${SHEET_NAME_VOTE}!A2:H2`; 
        ...
        row[index] = parseInt(row[index], 10) + 1; 
...
app.post("/clearvotes", async (req, res) => {
    try {
      const sheets = google.sheets({ version: "v4", auth: await auth.getClient() });
      const range = `${SHEET_NAME_VOTE}!A2:H2`;
      const emptyRow = Array(8).fill(0); // Set all values to 0
      ...
      res.status(500).send("Failed to clear votes");
    }
  });
...
app.post("/vibe", async (req, res) => {
  ...
  try {
    const sheets = google.sheets({ version: "v4", auth: await auth.getClient() });
    const range = `${SHEET_NAME_VIBE}!A2:A`; 
